...
def _set_env_label_and_glue_job_url():
    ...
    try:
        ...
        os.environ[NOTIFICATION_POSTFIX] = f"`{environment}` - {lambda_name}"
    except Exception as e:
        logger.error(
            f'Error setting notification prefix variable "{NOTIFICATION_POSTFIX}": {e}',
            exc_info=True,
        )
        raise e
...
def get_s3_object_key(event: dict) -> str:
    try:
        ...
        raise KeyError(
            f'Could not find S3 object key "{S3_OBJECT_KEY}" in Lambda event. Event received: "{event}"'
        )
    except Exception as e:
