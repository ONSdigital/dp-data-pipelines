...
def test_correct_columns_exist_incorrect_number_of_columns():
    ...
    assert (
        f"Number of columns in csv file ({expected_len}) does not match len of expected columns ({len(columns)})"
        in str(err.value)
    )
...
def test_dataframe_has_no_blanks_optional_columns_incorrect():
    ...
    assert (
        f"{optional_columns[0]} in check_specific_columns not found in dataframe"
        in str(err.value)
    )
...
def test_generated_dataframe_slices():
    fixture_file = Path(fixtures_files_dir / "test_validate_csv_data.csv")
    expected_columns_from_fixture_file = [
        "ID",
        "TIME_PERIOD",
        "OBS_VALUE",
        "OBS_STATUS",
    ...
    total_number_of_slices = (
        len_fixture_file // chunk_size
    )  # returns 3, will be counting using zero indexing
