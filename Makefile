...
symlink:
	...
	sed -i '' 's|{ git = "https://github.com/ONSdigital/dp-python-tools.git", tag = "v0.5.2" }|{path = "../dp-python-tools",  develop = true}|' pyproject.toml
	poetry lock
	poetry install
...
unlink:
	# Switch to production version
	sed -i '' 's|{path = "../dp-python-tools",  develop = true}|{ git = "https://github.com/ONSdigital/dp-python-tools.git", tag = "v0.5.2" }|' pyproject.toml
	poetry lock
	poetry install