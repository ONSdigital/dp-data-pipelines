...
def xmlToCsvSDMX2_1(input_path, output_path):
        ...
        for element in table.items():
            if element[0] == 'generic:SeriesKey':
                ...
                for observation in element[1]:
                    series_temp = series_dict
                    observation_temp = {}
                    df = pd.DataFrame(observation['generic:Attributes']['generic:Value'])
                    observation_temp.update({a:b for a,b in zip(df['@id'], df['@value'])})
                    observation_temp.update({'generic:ObsDimension': next(iter(observation['generic:ObsDimension'].values()))})
                    observation_temp.update({'generic:ObsValue': next(iter(observation['generic:ObsValue'].values()))})
    ...
    full_table.rename(columns={"generic:ObsDimension" : "TIME_PERIOD", "generic:ObsValue" : "OBS_VALUE"}, inplace=True)
