...
def check_header_info(header: dict):
    ...
    assert len(expected_keys) == len(header), f"was expecting header of size ({len(expected_keys)})"
...
def check_length_of_dict_is_expected_length(data_dict: dict, no_of_expected_obs: int):
    ...
    for key in data_dict:
        assert len(data_dict[key]) == no_of_expected_obs, f"expected length of {key} is not {no_of_expected_obs}"

def check_length_of_dataframe_is_expected_length(dataframe: pd.DataFrame, no_of_expected_obs: int):
    # checks if dataframe is the expected length
    # the expected length is the number of obs found using get_number_of_obs_from_xml_file()
    assert len(dataframe) == no_of_expected_obs, f"expected length of dataframe is not {no_of_expected_obs}"
...
def check_columns_of_dataframes_are_unique(
    ...
    for column in series_columns:
        assert column not in obs_columns, f"{column} is duplicated in series_frame and obs_frame"
        assert column not in header_columns, f"{column} is duplicated in series_frame and header_frame"
        
    for column in obs_columns:
        assert column not in header_columns, f"{column} is duplicated in obs_frame and header_frame"
