...
def check_header_info(header: dict):
    ...
    assert isinstance(header, dict), "function was expecting a dict"
...
def check_tables_list(xml_file: Path, tables_list: List[dict]):
    # check the tables list is a list of dicts
    assert isinstance(tables_list, list), f"expected 'tables' variable to be a list but got {type(tables_list)}" 
    for item in tables_list:
        assert isinstance(item, dict), f"expected item of list to be a dict but got {type(item)}"
...
def get_number_of_series(xml_file: Path):
...
def check_table_headers_are_consistent(expected_headers: List[str], found_headers: List[str]):
